...
export default function DashboardPage() {
  ...
  return (
    <AuthGate>
      <AppShell>
        <section className="patient-experience patient-list mx-auto max-w-5xl">
          <header className="patient-list-header flex items-center justify-between gap-4 px-1 pt-2">
          ...
          {!loading && patients.length > 0 ? <ul className="patient-list-grid grid gap-4">{patients.map((patient) => <li key={patient.id}><button type="button" onClick={() => router.push(`/patients/${patient.id}`)} className="patient-list-card group min-h-64 w-full border border-line bg-card p-5 text-left transition hover:-translate-y-1 hover:border-[#9dcac2]"><PatientAvatar patient={patient} /><div className="mt-8"><p className="patient-list-name font-semibold tracking-tight">{patient.name}</p><p className="patient-metadata mt-1 text-muted">{[patient.age != null ? `${patient.age} yrs` : null, patient.sex].filter(Boolean).join(" · ") || "No demographics recorded"}</p>{patient.relation && patient.relation !== "Self" ? <p className="patient-metadata mt-1 text-muted">{patient.relation}</p> : patient.relation === "Self" ? <p className="patient-metadata mt-1 text-muted">My profile</p> : null}</div><div className="patient-metadata mt-8 flex items-center justify-between border-t border-line pt-4 font-medium"><span>Open patient</span><span aria-hidden="true" className="text-lg transition-transform group-hover:translate-x-1">→</span></div></button></li>)}</ul> : null}</div>
        </section>
      </AppShell>
    </AuthGate>
  );
