...
def test_generate_schedule_spacing_defaults_to_even_daily_spread(
    ...
    assert (t1 - t0).total_seconds() == 12 * 3600
...
def test_generate_schedule_respects_explicit_interval_hours(
    ...
    medication = _create_medication(
        patient["id"], str(existing_drug_id), times_per_day=1, duration_days=2, interval_hours=8
    )
...
def test_generate_schedule_twice_returns_409(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
...
def test_generate_schedule_exceeding_max_doses_returns_400(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=24, duration_days=200)
    resp = client.post(f"/api/v1/medications/{medication['id']}/schedule")
    assert resp.status_code == 400
...
def test_generate_schedule_for_medication_owned_by_another_user_returns_404(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    app.dependency_overrides[get_current_user] = _override_current_user(uuid.uuid4())
    resp = client.post(f"/api/v1/medications/{medication['id']}/schedule")
    assert resp.status_code == 404
...
def test_generate_schedule_with_interval_hours_only_creates_expected_dose_count(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), duration_days=1, interval_hours=8)
...
def test_generate_schedule_with_interval_hours_only_spacing_matches_interval(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), duration_days=2, interval_hours=6)
    doses = sorted(_generate_schedule(medication["id"]), key=lambda d: d["scheduled_time"])
    t0 = datetime.fromisoformat(doses[0]["scheduled_time"].replace("Z", "+00:00"))
    t1 = datetime.fromisoformat(doses[1]["scheduled_time"].replace("Z", "+00:00"))
...
def test_generate_schedule_with_interval_hours_only_floors_partial_dose(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), duration_days=1, interval_hours=5)
    doses = _generate_schedule(medication["id"])
    assert len(doses) == 4
...
def test_generate_schedule_interval_hours_only_exceeding_max_doses_returns_400(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), duration_days=400, interval_hours=1)
    resp = client.post(f"/api/v1/medications/{medication['id']}/schedule")
    assert resp.status_code == 400
    assert "exceeding" in resp.json()["detail"]
...
def test_upcoming_doses_returns_future_unmarked_doses_ordered(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=3, duration_days=2)
    _generate_schedule(medication["id"])
...
def test_upcoming_doses_excludes_inactive_medication(
    ...
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=5)
    _generate_schedule(medication["id"])
    client.put(f"/api/v1/medications/{medication['id']}", json={"status": "discontinued"})
    resp = client.get(f"/api/v1/patients/{patient['id']}/doses/upcoming")
    assert resp.status_code == 200
    assert not any(d["medication_id"] == medication["id"] for d in resp.json())
...
def test_upcoming_doses_scoped_to_patient(
    ...
    medication_a = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=2)
    _generate_schedule(medication_a["id"])
    medication_b = _create_medication(other_patient["id"], str(existing_drug_id), times_per_day=1, duration_days=2)
...
def test_upcoming_doses_for_patient_owned_by_another_user_returns_404(existing_auth_user_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
...
def test_mark_dose_taken_sets_status_and_defaults_actual_time(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Mark Taken Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    resp = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "taken"})
    assert resp.status_code == 200
    marked = resp.json()
    assert marked["status"] == "taken"
    assert marked["actual_time"] is not None
    assert marked["updated_at"] != doses[0]["updated_at"]


def test_mark_dose_taken_respects_explicit_actual_time(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Mark Taken Explicit Time Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    explicit_time = "2026-07-31T09:15:00Z"
    resp = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "taken", "actual_time": explicit_time})
    assert resp.status_code == 200
    assert resp.json()["actual_time"] == explicit_time


def test_mark_dose_missed_leaves_actual_time_null(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Mark Missed Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    resp = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "missed"})
    assert resp.status_code == 200
    marked = resp.json()
...
def test_mark_dose_skipped_leaves_actual_time_null(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Mark Skipped Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    resp = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "skipped"})
    assert resp.status_code == 200
    marked = resp.json()
...
@pytest.mark.parametrize("mark_status,event_type", [("taken", "dose_taken"), ("missed", "dose_missed"), ("skipped", "dose_skipped")])
def test_mark_dose_logs_corresponding_timeline_event(existing_auth_user_id, existing_drug_id, created_patient_ids, mark_status, event_type):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
...
def test_mark_dose_twice_returns_409(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Mark Twice Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    first = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "taken"})
    assert first.status_code == 200
    second = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "missed"})
...
def test_mark_dose_owned_by_another_user_returns_404(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
...
def test_mark_dose_invalid_status_returns_422(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    ...
    medication = _create_medication(patient["id"], str(existing_drug_id), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    resp = client.post(f"/api/v1/doses/{doses[0]['id']}/mark", json={"status": "snoozed"})
...
def test_upcoming_doses_sweeps_overdue_unmarked_doses_to_missed(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Sweep Via Upcoming Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    past_start = date.today() - timedelta(days=2)
    medication = _create_medication(patient["id"], str(existing_drug_id), start_date=str(past_start), times_per_day=2, duration_days=1)
    doses = _generate_schedule(medication["id"])
    ...
    events = _get_timeline(patient["id"])
    missed_events = [e for e in events if e["event_type"] == "dose_missed" and e["ref_id"] in dose_ids]
    assert len(missed_events) == len(dose_ids)
    assert all(e["payload"]["auto_detected"] is True for e in missed_events)


def test_mark_dose_sweeps_overdue_dose_before_processing_the_mark(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    patient = _create_patient("Sweep Via Mark Patient")
    created_patient_ids.append(uuid.UUID(patient["id"]))
    past_start = date.today() - timedelta(days=3)
    medication = _create_medication(patient["id"], str(existing_drug_id), start_date=str(past_start), times_per_day=1, duration_days=1)
    doses = _generate_schedule(medication["id"])
    dose_id = doses[0]["id"]
    ...
    events = _get_timeline(patient["id"])
    missed_events = [e for e in events if e["event_type"] == "dose_missed" and e["ref_id"] == dose_id]
    assert len(missed_events) == 1


def test_sweep_does_not_affect_future_doses(existing_auth_user_id, existing_drug_id, created_patient_ids):
    app.dependency_overrides[get_current_user] = _override_current_user(existing_auth_user_id)
    ...
    mark_resp = client.post(f"/api/v1/doses/{doses[-1]['id']}/mark", json={"status": "taken"})
