...
**Implementation status:** shipped. `backend/scripts/import_rxnorm.py` now defaults to RxNorm Current Prescribable Content (`/REST/Prescribe/allconcepts.json?tty=IN`) — structurally satisfying the "active only," "no obsolete," "no veterinary-only," "no allergenic-extract" requirements via NLM's curation. Full catalog remains available via explicit `--full-rxnorm` (`/REST/allconcepts.json`). Parsing, transformation, and persistence are streaming/bounded-memory via `ijson` and batched writes (configurable `rxnorm_import_batch_size=500`); the HTTP download itself still lands on disk whole (RxNav has no pagination). No `term_type`/`is_active`/`rxnorm_term_type_enum`/`lower(name)` index changes are included — those remain future work.
...
Do NOT claim `term_type`/`is_active`/`rxnorm_term_type_enum`/`lower(name)` index are shipped — those remain future work and are documented as such in §20.14.
