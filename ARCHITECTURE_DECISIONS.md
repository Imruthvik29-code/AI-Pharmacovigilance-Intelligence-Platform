...
- Migrations are flat, sequentially numbered SQL files at the repository root (`001_initial_schema.sql`, `002_seed_data.sql`, `003_reference_drugs_external_reference.sql`) as the pre-Alembic baseline. Alembic (`backend/alembic/`) now manages versioned migrations, with history `0001_baseline -> 0002_add_term_type_is_active`.
...
**Implementation status:** shipped. Alembic migration `0002_add_term_type_is_active` (revises `0001_baseline`) creates `rxnorm_term_type_enum` seeded with the full 23-value NLM Appendix 5 TTY vocabulary, adds nullable `reference_drugs.term_type` (`rxnorm_term_type_enum`), and adds `reference_drugs.is_active` (`boolean NOT NULL DEFAULT true`). The importer remains `IN`-only and does not yet populate `term_type`/`is_active` (see `backend/scripts/README.md`).
...
**Final decision:** the importer sources **Prescribable Content by default, with `--full-rxnorm` as an explicit fallback** for the full catalog. This structurally satisfies the "active only," "no obsolete," "no veterinary-only," and "no allergenic-extract" exclusion requirements using NLM's own maintained curation by default, while still allowing the full `/REST/allconcepts.json` catalog via an explicit opt-in, rather than a custom keyword-blocklist approach (rejected — see Section 20, Part 2 — not yet authored).

The `term_type`/`is_active` columns and `rxnorm_term_type_enum` type now ship via migration `0002_add_term_type_is_active` (the Phase 1 additions described in Section 7.3); the importer does not yet populate them, and no `lower(name)` index is introduced in this phase.
...
**Implementation status:** shipped. Migration `0002_add_term_type_is_active` (revises `0001_baseline`) adds the `term_type` and `is_active` columns above; Alembic history is now `0001_baseline -> 0002_add_term_type_is_active`. The importer remains `IN`-only and does not yet populate `term_type`; every row, including legacy rows, has `is_active = true` via the server default.
...
| **Seeded Phase 1 additions (`term_type`/`is_active`) ship via `0002_add_term_type_is_active`** | No test seeds `term_type`/`is_active` Phase 1 additions (columns now ship via migration `0002_add_term_type_is_active`; no test seeds them yet) — `VERIFIED (repository: `grep -n "term_type" backend/tests/` → `0` beyond docstring)** |
...
### 17.6 Migrations — three sequential SQL files as baseline, now managed by Alembic

**VERIFIED (repository: `ls *.sql` + `README.md` Database setup + `ARCHITECTURE_DECISIONS.md:107` + `ls backend/alembic/versions/`):**

**The repository contains three sequential SQL migration files.** The README instructs operators to apply them manually in order. **The repository now uses Alembic** (`alembic.ini` at repo root + `backend/alembic/`), which has taken over versioned migration management from the three manual SQL files; `0001_baseline` represents the pre-Alembic schema and `0002_add_term_type_is_active` is the first forward migration.
...
| **Repository:** manual instruction | `README.md` Database setup: “*Before starting the backend for the first time, run the SQL migrations against your Supabase project (SQL editor or `psql`), in order:*” — these three files remain the manual baseline, stamped as `0001_baseline`; forward migrations now run via `alembic upgrade head` (`backend/alembic/`) — `VERIFIED (repository)` |
| **Repository:** Alembic adopted | `ls backend/alembic/versions/` → `0001_baseline_pre_alembic_manual.py` + `0002_add_term_type_is_active.py` — **The repository now uses Alembic** — `VERIFIED (repository)` for presence of `backend/alembic/` |
...
- **Migration tooling** — Alembic now adopted (`alembic.ini` + `backend/alembic/` with history `0001_baseline -> 0002_add_term_type_is_active`) — `VERIFIED (repository: `ls backend/alembic/versions/` → `0001_baseline_pre_alembic_manual.py` + `0002_add_term_type_is_active.py`)` — three sequential SQL files remain the manual baseline (`001`/`002`/`003`), stamped as `0001_baseline`.
...
Future work consists of **additive evolution rather than architectural redesign**. Deferred items include **RxNorm expansion, pgvector retrieval, indexing improvements, scalability enhancements, frontend completion, deployment, and operational capabilities**. Their implementation order will be documented in a **future roadmap section** — `VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:6.4` “*Future work consists of additive evolution rather than architectural redesign*” — additive evolution principle per `ARCHITECTURE_DECISIONS.md:2` design principles + `6.4` early Alembic adoption while count low (now shipped), `7.2` TTY `PIN` deferred/`MIN`+`BN` blocked until decomposition, `6.3` `pg_trgm` deferred, `evidence_retrieval.py:12` `pgvector added later without node changes`, `backend/app/core/config.py` single `DATABASE_URL` scalability placeholder, `PROJECT_PHASES.md` Milestone 5 `Phase 16/17` unchecked, `ARCHITECTURE_DECISIONS.md:18` `The repository currently contains no evidence of centralized logging/rate limiting/monitoring`)*.*
...
|---|---|---|
| **Backend — data layer** | `term_type`/`is_active` columns + `rxnorm_term_type_enum` type now shipped via `0002_add_term_type_is_active` (nullable `term_type`, `is_active boolean not null default true`); `IN`-only import (`PIN` deferred, `MIN`/`BN` blocked until decomposition — see §19.3) | `VERIFIED (repository: `6.2` + `7.2` + `7.3` `Implementation status: shipped via 0002`)` |
...
| **Backend — migration & scalability** | Migration tooling (`Alembic`) adopted (`0001_baseline -> 0002_add_term_type_is_active`) — `6.4`; single `DATABASE_URL` with no `REDIS`/`CELERY`/`POOLER`/`PARTITION`/`CACHE` — `config.py:32-55` only `database_url` + `grep REDIS\|CELERY\|POOLER\|PARTITION\|CACHE` → `0` | `VERIFIED (repository: `grep` → `0`)` |
...
**VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:6.4` + `ls *.sql` → 3 + `ls backend/alembic/versions/` → `0001_baseline_pre_alembic_manual.py` + `0002_add_term_type_is_active.py` + `Alembic` docs — `VERIFIED (official documentation)`):**

`ARCHITECTURE_DECISIONS.md:6.4` “*Accepted correction to prior phasing: migration tooling adoption … was originally placed in a late-stage ‘enterprise scale’ phase … **Final decision:** … should occur while migration-file count remains low (early), not deferred to a late phase*” — the cost of adopting tracked migrations is proportional to untracked history at adoption time; the repository currently contains **three sequential SQL migration files** (`001_initial_schema.sql`, `002_seed_data.sql`, `003_reference_drugs_external_reference.sql` — `ls *.sql` → 3) applied manually via `psql`/SQL editor — `VERIFIED (repository: `6.4` + `ls *.sql`)`. **The repository now uses Alembic** (`alembic.ini` at repo root + `backend/alembic/` with `env.py`/`script.py.mako`/`versions/0001_baseline_pre_alembic_manual.py` + `versions/0002_add_term_type_is_active.py`) — `VERIFIED (repository: `ls backend/alembic/versions/` → `0001_baseline_pre_alembic_manual.py` + `0002_add_term_type_is_active.py`)` and `VERIFIED (official documentation)` for the `Alembic` `alembic init` file layout (now present).
...
- **RxNorm** — `PIN` deferred, `MIN`/`BN` blocked until decomposition; `term_type`/`is_active` and `rxnorm_term_type_enum` now ship via `0002_add_term_type_is_active` (importer still `IN`-only, does not populate `term_type`).
...
**The approved architectural decision is Prescribable Content by default, with `--full-rxnorm` as an explicit fallback for the full catalog** — `VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:122` “*Final decision: the importer sources Prescribable Content by default, with `--full-rxnorm` as an explicit fallback*” — now phrased as Prescribable by default, not exclusively, with `--full-rxnorm` opt-in — this structurally satisfies the “active only,” “no obsolete,” “no veterinary-only,” and “no allergenic-extract” requirements using NLM’s own maintained curation on the default path, while still allowing full `/REST/allconcepts.json` via explicit fallback, rather than a custom keyword-blocklist — `VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:122` final decision)**.
...
The database decision in `§6.2` reflects this: `reference_drugs.term_type` is `rxnorm_term_type_enum` seeded with the **full documented RxNorm TTY vocabulary** (`IN, PIN, MIN, BN, SCD, SBD, SCDC, SCDF, SCDFP, SCDG, SCDGP, SBDC, SBDF, SBDFP, SBDG, GPCK, BPCK, DF, DFG, ET, PSN, SY, TMSY`) per NLM Appendix 5 — even though only `IN` is imported in Phase 1 (see §20.5/§20.12) — `VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:6.2` + `95` full vocabulary + `backend/alembic/versions/0002_add_term_type_is_active.py` → present — now shipped)** and `VERIFIED (official documentation)` for **NLM Appendix 5 TTY list where such verification has been performed and documented**.

### 20.5 Implementation status — Prescribable Content now shipped as default (with --full-rxnorm fallback)

**VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:7.1` + `backend/scripts/import_rxnorm.py` `RXNAV_BASE_URL` + `grep -n "Prescribe" backend/scripts/import_rxnorm.py` + `grep -n "allconcepts.json" backend/scripts/import_rxnorm.py`):**

`backend/scripts/import_rxnorm.py` now defaults to `RXNAV_BASE_URL + "/Prescribe/allconcepts.json"` (`https://rxnav.nlm.nih.gov/REST/Prescribe/allconcepts.json?tty=IN` — Prescribable Content by default) with an explicit `--full-rxnorm` fallback to `"/allconcepts.json"` — `VERIFIED (repository: `backend/scripts/import_rxnorm.py` `def _rxnav_url` returns `Prescribe/allconcepts.json` when `full_rxnorm=False` else `allconcepts.json` + `grep -n "Prescribe" backend/scripts/import_rxnorm.py` → `Prescribe/allconcepts.json` present + `grep -n "_full-rxnorm\|full_rxnorm" backend/scripts/import_rxnorm.py` → flag wired + `ARCHITECTURE_DECISIONS.md:7.1` shipped status)**. The Prescribable exclusion guarantees (no obsolete/suppressed, no veterinary-only, no allergenic-extract, `SAB=RXNORM`+`MTHSPL`) are now structurally enforced on the default path; `--full-rxnorm` bypasses that filtering as an explicit fallback/full-catalog mode. Streaming/bounded-memory applies to parsing (`ijson`), transformation, and persistence (batched writes, `rxnorm_import_batch_size=500`); the HTTP download still lands on disk whole (RxNav has no pagination) — accurate bounded claim per §7.1.

`term_type`/`is_active`/`rxnorm_term_type_enum` now ship via migration `0002_add_term_type_is_active`; the `lower(name)` index remains future work and is documented as such in §20.14.
...
### 20.14 Current limitations and implementation status — `term_type`/`is_active` now shipped (`0002_add_term_type_is_active`); `UNVERIFIED` items remain
...
|---|---|---|---|---|
| `reference_drugs.term_type` (`rxnorm_term_type_enum` with full NLM Appendix 5 vocabulary `IN…TMSY`) | Approved per §6.2 — `rxnorm_term_type_enum` seeded with **full** Appendix 5 vocabulary, with the importer’s **allow-list** enforcing `IN`-only for Phase 1 | **Shipped** — migration `0002_add_term_type_is_active` creates the `rxnorm_term_type_enum` type and the nullable `term_type` column; `backend/scripts/README.md` now states the columns exist and the importer does not populate them | **Shipped** — `VERIFIED (repository: `backend/alembic/versions/0002_add_term_type_is_active.py` creates `rxnorm_term_type_enum` + `term_type` + `is_active` + `7.3` table `term_type` row + `grep -n "term_type" 001_initial_schema.sql` → `0` (added by `0002`, not `001`))` | — |
| `reference_drugs.is_active` (`boolean not null default true`) | Approved per §6.2/§7.3 — every row active until proven otherwise; `NULL` not allowed | **Shipped** — same as `term_type` (migration `0002_add_term_type_is_active`) — `VERIFIED (repository: `6.2` + `7.3` `Implementation status: present` + `backend/alembic/versions/0002_add_term_type_is_active.py`)` | **Shipped** | — |
...
| RxNorm source (`Prescribe` vs full) | Approved per §7.1 Prescribable default + `--full-rxnorm` fallback | **Shipped** — `backend/scripts/import_rxnorm.py` defaults to `Prescribe/allconcepts.json?tty=IN` (Prescribable) with `--full-rxnorm` → `allconcepts.json`; streaming (`ijson`), batch writes (`rxnorm_import_batch_size=500`), atomic `.partial`/`checkpoint` — `VERIFIED (repository: `grep -n "Prescribe" import_rxnorm.py` → `Prescribe/allconcepts.json` + `grep -n "full_rxnorm" import_rxnorm.py` → flag + `grep -n "ijson" import_rxnorm.py` → streaming + `config.py: rxnorm_import_batch_size`) | **Shipped — Prescribable by default, full-catalog via explicit flag; bounded-memory parsing/transformation/persistence (download lands on disk)** | `ARCHITECTURE_DECISIONS.md:7.1` |
...
*Migration `0002_add_term_type_is_active` creates `rxnorm_term_type_enum`, `term_type`, and `is_active` — `grep -n "rxnorm_term_type_enum\|is_active" 001_initial_schema.sql` → `0` (these are added by `0002`, not the baseline `001`).*
...
| **Implementation status** | `term_type`/`is_active` columns + `rxnorm_term_type_enum` **shipped** (migration `0002_add_term_type_is_active`) — `VERIFIED (repository: `6.2`/`7.3` + `grep term_type` → `0` in `001_initial_schema.sql` (added by `0002`))` | `--dry-run` measured count remains **approved but not yet implemented** — RxNorm source now **shipped** (Prescribable default + `--full-rxnorm` fallback, streaming/batched, atomic cache/checkpoint) — `VERIFIED (repository: `ARCHITECTURE_DECISIONS.md:7.1` shipped + `backend/scripts/import_rxnorm.py` `Prescribe/allconcepts.json` + `ijson` + `rxnorm_import_batch_size`)` | `backend/scripts/README.md` (RxNorm) vs `001_initial_schema.sql` (term_type) |
...
| **Implemented behavior vs approved-but-not-implemented** | `11` indexes exist, `MAX_GENERATED_DOSES=3650` cap exists, `no LIMIT` on timeline/analysis history (full list), `ENGINEERING ESTIMATE` pending `--dry-run`, 3 sequential SQL migrations manual as baseline + **Alembic adopted** (`0001_baseline -> 0002_add_term_type_is_active`) — `VERIFIED (repository)` | `Alembic` `alembic init` layout — `VERIFIED (official documentation)` for `Alembic` (now present) | `Alembic` adoption while count low (3 files) — **SHIPPED** — `§19.4` |
...
- CI/CD: **The repository currently contains no evidence of GitHub Actions workflows that execute tests or deployments automatically** — `VERIFIED (repository: ls .github/workflows/ → No such file)`
- Migration tooling: **Alembic now adopted** — `alembic.ini` + `backend/alembic/` with history `0001_baseline -> 0002_add_term_type_is_active` — `VERIFIED (repository: ls backend/alembic/versions/ → 0001_baseline_pre_alembic_manual.py + 0002_add_term_type_is_active.py)`
...
**Objective (SHIPPED):** Introduce Alembic migration framework.

- **Reason:** Repository contains 3 sequential SQL files (`001_initial_schema.sql`, `002_seed_data.sql`, `003_reference_drugs_external_reference.sql`) applied manually via `psql`/SQL editor — Alembic now adopted (`alembic.ini` + `backend/alembic/`, history `0001_baseline -> 0002_add_term_type_is_active`) — cost of adopting tracked migrations grows with untracked history.
...
**Objective (SHIPPED):** Migrate `reference_drugs.term_type` (`rxnorm_term_type_enum` full Appendix 5 `IN…TMSY`) + `is_active boolean not null default true` — now shipped via `0002_add_term_type_is_active`.

- **Reason:** Approved in §6.2/§7.3 and now shipped — migration `0002_add_term_type_is_active` creates `rxnorm_term_type_enum` + nullable `term_type` + `is_active boolean not null default true` (the columns did not exist before `0002`) — `VERIFIED (repository)` for presence of `0002`.
...
**Objective:** Replace engineering estimate with measured Prescribable count via `--dry-run` (importer now shipped).

- **Reason:** `backend/scripts/import_rxnorm.py` now defaults to `Prescribe/allconcepts.json?tty=IN` with `--full-rxnorm` fallback — `VERIFIED (repository: §7.1 shipped + `grep Prescribe import_rxnorm.py` → `Prescribe/allconcepts.json`)` — streaming/batched/persisted with `rxnorm_import_batch_size=500`, atomic `.partial`/checkpoint — but catalog size remains `ENGINEERING ESTIMATE 4,000–6,000` based on 2013 baseline 4,320 until measured via `--dry-run` (see §20.14).
- **Dependencies:** `After` Prescribable ship (done), `Before` PIN expansion — `Requires` live `--dry-run` against `Prescribe/allconcepts.json`.
- **Success Criteria:** `--dry-run` outputs measured `IN`-only Prescribable count (replaces `ENGINEERING ESTIMATE`), no obsolete/suppressed/veterinary-only/allergenic concepts on default path, `lower(name)` backfill preserved, streaming/batched behavior retained.
...
|---|---|---|---|
| Alembic migration framework | **Shipped** — `alembic.ini` + `backend/alembic/` present; history `0001_baseline -> 0002_add_term_type_is_active` (§6.4) | `Independent`, `Before` any schema extension (done) | `alembic/` present, versioned history, upgrade/downgrade reproducible |
| `reference_drugs.term_type` (`rxnorm_term_type_enum` full `IN…TMSY`) + `is_active` | **Shipped** — migration `0002_add_term_type_is_active` creates enum + columns; seed rows preserved, `rxcui` upsert still works — `backend/alembic/versions/0002_add_term_type_is_active.py` | `After` Alembic (done), `Before` PIN/MIN | Migration creates enum + columns, seed rows preserved, `rxcui` upsert still works |
| Prescribable Content importer `Prescribe/allconcepts.json` | **Shipped** — defaults to `Prescribe/allconcepts.json?tty=IN` with `--full-rxnorm` fallback — `grep Prescribe import_rxnorm.py` → `Prescribe/allconcepts.json` + streaming/batched/atomic — `VERIFIED (repository: §7.1 shipped)` | `After` term_type migration (for future PIN/MIN) | Prescribable now default; remaining is measured `--dry-run` count to replace `ENGINEERING ESTIMATE 4,000–6,000` |
...
|---|---|---|
| **Migrations & schema** | 3 SQL files manual as baseline + Alembic now adopted — `VERIFIED (repository)` for presence of `backend/alembic/` + `0001_baseline -> 0002_add_term_type_is_active` — `term_type`/`is_active` shipped via `0002` (§6.2) — `grep term_type 001_initial_schema.sql` → 0 (added by `0002`, not `001`) | Alembic adoption + term_type/is_active migration now shipped — `Before` PIN/MIN |
